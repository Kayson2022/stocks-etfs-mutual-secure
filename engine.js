        ...
        class ApiService {
            ...
            // --- Yahoo Finance Mutual Fund Quote helper (direct v7/finance/quote) ---
            async fetchYahooFundData(symbol) {
                try {
                    const url = `https://corsproxy.io/?https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(symbol)}`;
                    const response = await fetch(url);
                    if (!response.ok) {
                        return { success: false, symbol, reason: `Yahoo HTTP Error: ${response.status}` };
                    }
                    const json = await response.json();
                    const quote = json?.quoteResponse?.result?.[0];
                    if (!quote || quote.regularMarketPrice == null) {
                        return { success: false, symbol, reason: 'Yahoo: Empty or invalid quote payload for mutual fund.' };
                    }
                    ...
                    return {
                        ...
                        data: {
                            ...
                            change,
                            changePercent,
                            open: quote.regularMarketOpen != null ? Number(quote.regularMarketOpen) : 0,
                            high: quote.regularMarketDayHigh != null ? Number(quote.regularMarketDayHigh) : price,
                            low: quote.regularMarketDayLow != null ? Number(quote.regularMarketDayLow) : price,
                            previousClose,
                            industry: 'Mutual Fund'
                        }
                    };
                } catch (err) {
                    console.warn(`Yahoo mutual fund fetch failed for ${symbol}`, err);
                    return { success: false, symbol, reason: err.message || 'Yahoo mutual fund fetch failed.' };
                }
            }
            ...
            async fetchUnifiedStockQuote(symbol) {
                ...
                try {
                    ...
                    return {
                        ...
                        usedApi: "backend",
                        data: {
                            percentChange: d.changePercent ?? d.percentChange,
                            changePercent: d.changePercent ?? d.percentChange,
                            change: d.change,
...
async fetchStockEtfQuote(symbol, rangeKey = "1D") {
    ...
    try {
        ...
        // --- FORCE MATHEMATICAL CONSISTENCY BETWEEN PRICE, CHANGE, AND PERCENT ---
        ...
        if (typeof payload.last === "number") {
            ...
            const parsedLast = Number(payload.last);
            price = Number.isFinite(parsedLast) ? parsedLast : 0;
        }

        // 2. Normalize percent change (this is usually the most reliable field)
        let percentChange = 0;
        if (typeof payload.changePercent === "number") {
            percentChange = payload.changePercent;
        } else if (payload.changePercent != null) {
            const parsedPercent = Number(String(payload.changePercent).replace("%", ""));
            if (Number.isFinite(parsedPercent)) {
                percentChange = parsedPercent;
            }
        }

        // 3. Back-calculate the TRUE previous close when possible
        let previousClose = 0;
        if (price !== 0 && percentChange !== 0) {
            previousClose = price / (1 + (percentChange / 100));
        } else {
            // Fallback to backend values if percent is missing/zero
            const candidates = [payload.previousClose, payload.chartPreviousClose];
            ...
            if (!previousClose) {
                previousClose = price; // final fallback
            }
        }

        // 4. Compute dollar change strictly from price and our corrected previous close
        let change = price - previousClose;

        // 5. If percentChange was zero but backend provided a usable change, recompute percentChange
        if (!percentChange && previousClose) {
            let backendChange = 0;
            if (typeof payload.change === "number") {
                backendChange = payload.change;
            } else if (payload.change != null) {
                const parsedChange = Number(payload.change);
                backendChange = Number.isFinite(parsedChange) ? parsedChange : 0;
            }
        ...
        // 6. High/low are optional; if the backend didn't provide them, fall back to price.
        const dayHigh =
            typeof payload.dayHigh === "number" && Number.isFinite(payload.dayHigh)
                ? payload.dayHigh
                : price;
        const dayLow =
            typeof payload.dayLow === "number" && Number.isFinite(payload.dayLow)
                ? payload.dayLow
                : price;
        ...
        return { success: false, symbol, reason: `Backend Error: ${error.message}` };
            ...
            // --- Alpha Vantage Fetch (Last Resort for Mutual Funds) ---
                        ...
                        // --- Alpha Vantage Fetch (Mutual Fund Quote helper) ---
            async fetchAlphaVantageFundData(symbol) {
                const key = this.config.alphaVantageKey;
                ...
                try {
                    ...
                    const response = await fetch(url);
                    if (!response.ok) {
                        return { success: false, symbol, reason: `Alpha Vantage HTTP Error: ${response.status}` };
                    }
                    const json = await response.json();
                    const q = json["Global Quote"] || json["GlobalQuote"] || null;
                    if (!q || !q["05. price"]) {
                        return { success: false, symbol, reason: 'Alpha Vantage: Invalid or empty payload.' };
                    }

                    const price = parseFloat(q["05. price"]) || 0;
                    const previousClose = parseFloat(q["08. previous close"]) || price;
                    const change = parseFloat(q["09. change"]) || (price - previousClose);
                    const percentRaw = q["10. change percent"] || '';
                    const changePercent = parseFloat(String(percentRaw).replace('%', '')) || (previousClose ? (change / previousClose) * 100 : 0);
                    ...
                    console.warn(`Alpha Vantage fund fetch failed for ${symbol}`, err);
                    return { success: false, symbol, reason: err.message || 'Alpha Vantage fund fetch failed.' };
                }
            }

            // --- Marketstack Fetch (Mutual Fund Quote helper) ---
            async fetchMarketstackFundData(symbol) {
                const key = this.config.marketstackKey;
                if (!key || key === 'YOUR_MARKETSTACK_API_KEY') {
                    return { success: false, symbol, reason: 'Marketstack API Key missing.' };
                }
                try {
                    const url = `https://api.marketstack.com/v1/eod?access_key=${key}&symbols=${encodeURIComponent(symbol)}&limit=1`;
                    const response = await fetch(url);
                    if (!response.ok) {
                        return { success: false, symbol, reason: `Marketstack HTTP Error: ${response.status}` };
                    }
                    const json = await response.json();
                    const dataArr = (Array.isArray(json.data) && json.data.length)
                        ? json.data
                        : (Array.isArray(json.eod) && json.eod.length ? json.eod : []);
                    if (!dataArr.length) {
                        return { success: false, symbol, reason: 'Marketstack: Empty payload.' };
                    }
                    const d = dataArr[0];
                    const price = Number(d.close) || 0;
                    const previousClose = Number(d.previous_close != null ? d.previous_close : d.close) || price;
                    ...
                    console.warn(`Marketstack fund fetch failed for ${symbol}`, err);
                    return { success: false, symbol, reason: err.message || 'Marketstack fund fetch failed.' };
                }
            }

// --- Fallback Function for Mutual Funds (STRICTLY Yahoo -> AV -> Marketstack -> FMP) ---
            async fetchMutualFundDataWithFallback(symbol) {
                let lastError = null;

                // 0. If this is not explicitly a mutual fund pattern, try stock/ETF pipeline first
                const quoteResult = await this.fetchStockEtfQuote(symbol);
                ...
                // 1. Yahoo Finance direct (no API key required)
                try {
                    const yahooResult = await this.fetchYahooFundData(symbol);
                    if (yahooResult && yahooResult.success && yahooResult.data && yahooResult.data.price > 0) {
                        return yahooResult;
                    }
                    if (yahooResult && !yahooResult.success) {
                        lastError = yahooResult.reason || lastError;
                    }
                } catch (err) {
                    console.warn(`Yahoo mutual fund fetch failed for ${symbol}`, err);
                ...
                // 2. Alpha Vantage
                try {
                    const alphaResult = await this.fetchAlphaVantageFundData(symbol);
                    if (alphaResult && alphaResult.success && alphaResult.data && alphaResult.data.price > 0) {
                        return alphaResult;
                    }
                    if (alphaResult && !alphaResult.success) {
                        lastError = alphaResult.reason || lastError;
                    }
                } catch (err) {
                    console.warn(`Alpha Vantage mutual fund fetch failed for ${symbol}`, err);
                    lastError = err.message || lastError;
                }

                // 3. Marketstack
                try {
                    const msResult = await this.fetchMarketstackFundData(symbol);
                    if (msResult && msResult.success && msResult.data && msResult.data.price > 0) {
                        return msResult;
                    }
                    if (msResult && !msResult.success) {
                        lastError = msResult.reason || lastError;
                    }
                } catch (err) {
                    console.warn(`Marketstack mutual fund fetch failed for ${symbol}`, err);
                    lastError = err.message || lastError;
                }

                // 4. FMP (last resort, can be limited/403)
                try {
                    const fmpResult = await this.fetchFmpFundData(symbol);
                    if (fmpResult && fmpResult.success && fmpResult.data && fmpResult.data.price > 0) {
                        return fmpResult;
                    }
                    if (fmpResult && !fmpResult.success) {
                        lastError = fmpResult.reason || lastError;
                    }
                } catch (err) {
                    console.warn(`FMP mutual fund fetch failed for ${symbol}`, err);
                    lastError = err.message || lastError;
                }

                console.warn(`All mutual fund providers failed for ${symbol}. Last error: ${lastError || 'Unknown failure'}`);
                return { success: false, symbol, reason: lastError || 'All mutual fund providers failed.' };
            }
        ...
        class StockTracker {
            ...
            async loadData() {
                ...
                if (docSnap.exists()) {
                    ...
                    if (migrationNeeded) {
        // --- SANITIZER: Fix inconsistent previousClose/change using percentChange ---
        const sanitize = (item) => {
            if (!item) return;
            const p = Number(item.price) || 0;
            let pct = Number(item.changePercent);
            if ((!pct || Number.isNaN(pct)) && item.percentChange != null) {
                pct = Number(item.percentChange);
            }
            pct = pct || 0;
            if (p > 0 && pct !== 0) {
