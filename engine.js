/**
 * engine.js - helpers + core app logic
 * - getRgbColor: CSS color helper for charts
 * - StockTracker and related logic moved out of index.html
 */
...
(function () {
    ...
    console.log("engine.js helper loaded (with core app logic).");
})();

// ---- Core App Logic (moved from index.html) ----
...
async fetchFmpFundHistory(symbol, range) {
            ...
            async fetchStockEtfQuote(symbol, rangeKey = "1D") {
                ...
                try {
                    ...
                    // --- Unified Quote Normalization (Option A priority) ---
                    // 1) Normalize price (last trade)
                    const price = Number(
                        payload.last ??
                    ...
                    // 2) Choose the best "previous close" value
                    let previousClose = Number(
                        payload.chartPreviousClose ??
                        payload.regularMarketPreviousClose ??
                    ...
                    // 3) Rebuild change and percentChange so they are always consistent
                    const change = price - previousClose;
                    const percentChange = previousClose ? (change / previousClose) * 100 : 0;

                    // 4) High/Low fallbacks
                    const high = Number(payload.dayHigh ?? price);
                    const low = Number(payload.dayLow ?? price);
                    ...
                    console.error(`Error fetching stock/ETF data for ${symbol} (backend):`, error);
