...
class StockTracker {
    constructor() {
        ...
        // --- API keys + refresh config (loaded from localStorage) ---
        this.apiKeys = {
            finnhub: localStorage.getItem("api_finnhub") || "",
            fmp: localStorage.getItem("api_fmp") || "",
            marketstack: localStorage.getItem("api_marketstack") || "",
            alpha: localStorage.getItem("api_alpha") || "",
            refreshSeconds: Number(localStorage.getItem("api_refresh")) || 60
        };

        // --- Simple refresh loop flags ---
        this.refreshIntervalMs = this.apiKeys.refreshSeconds * 1000;
        this.refreshIntervalId = null;
        this.countdownId = null;
        this.countdownRemaining = this.apiKeys.refreshSeconds;
    ...
    startAutoRefresh() {
        ...
        this.refreshIntervalId = setInterval(() => {
            // When the timer hits, trigger a data refresh and reset countdown
            this.refreshAllPrices();
            this.countdownRemaining = this.apiKeys.refreshSeconds;
    ...
    startCountdown() {
        ...
        // Initialize remaining based on current config
        this.countdownRemaining = this.apiKeys.refreshSeconds;
        ...
        this.countdownId = setInterval(() => {
            ...
            if (this.countdownRemaining > 0) {
                headerCountdown.textContent = this.countdownRemaining + "s";
                this.countdownRemaining -= 1;
            } else {
                // Show refreshing state briefly while fetch runs
                headerCountdown.textContent = "Refreshing…";
            }
        }, 1000);
    }
    ...
    updateApiConfig(config) {
        ...
        this.apiKeys.finnhub = config.finnhub ?? this.apiKeys.finnhub;
        this.apiKeys.fmp = config.fmp ?? this.apiKeys.fmp;
        this.apiKeys.marketstack = config.marketstack ?? this.apiKeys.marketstack;
        this.apiKeys.alpha = config.alpha ?? this.apiKeys.alpha;

        if (typeof config.refreshSeconds === "number" && config.refreshSeconds > 0) {
            this.apiKeys.refreshSeconds = config.refreshSeconds;
            this.refreshIntervalMs = this.apiKeys.refreshSeconds * 1000;
        }

        // Restart loops with new interval
        this.startAutoRefresh();
        this.startCountdown();
    }
    ...
    async fetchQuoteWithFallback(symbol) {
        // Use keys from settings (localStorage-driven, or backend in future)
        const FINNHUB_KEY = (this.apiKeys && this.apiKeys.finnhub) || "";
        const FMP_KEY = (this.apiKeys && this.apiKeys.fmp) || "";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Start auto-refresh + countdown once ready
    if (window.stockTracker) {
        window.stockTracker.startAutoRefresh();
